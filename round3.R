...
read.df <- function(years, path="/Users/yuanwong/Downloads/focasting/") {
  df_comb <- data.frame()
  for (year in years) { 
...
additional1 <- read.csv("/Users/yuanwong/Downloads/focasting/2092252.csv",header=TRUE,sep=",")
...
additional <- read.csv("/Users/yuanwong/Downloads/focasting/2029335.csv",header=TRUE,sep=",")
...
wc[wc<0] <- 0
#plot(AWND,Vermont)
#wc <- 35.74+0.615*DryBulb-35.75*(AWND)^0.16+0.4275*DryBulb*(AWND)^0.16
...
read.data.day <- function(years, path="/Users/yuanwong/Downloads/focasting/") {
...
naive3.train <- window(naive3,end=c(2016,364))
naive3.valid <- window(naive3,start=c(2017,0),end=c(2018,364))
...
Vermont.train <- df.train_regress$Vermont
Date.train <- df.train_regress$Date

# Plot training time series 
# Slight overall decline (decreasing trend): hint of non-stationarity

plot(VT.train, ylab="Vermont daily peak demand")

# Checking for variance stability

VT.train.mean <- applySeries(VT.train,by="monthly",FUN=colMeans)
VT.train.sd <- applySeries(VT.train,by="monthly",FUN=colSds)
plot(series(VT.train.mean),series(VT.train.sd),
     main="Before Box-Cox transformation")

# Box Cox to stablize the variance

lambda <- BoxCox.lambda(VT.train)
VT.train.mean.bc <- applySeries(BoxCox(VT.train,lambda),by="monthly",FUN=colMeans)
VT.train.sd.bc   <- applySeries(BoxCox(VT.train,lambda),by="monthly",FUN=colSds)

plot(series(VT.train.mean.bc),series(VT.train.sd.bc),
     main=paste("For Vermont daily peak demand after Box-Cox, lambda=",
                round(lambda,2)))

VT.train.bc <- ((VT.train^lambda)-1)/lambda
VT.valid.bc <- ((VT.valid^lambda)-1)/lambda

plot(VT.train.bc)

# ACF and PACF do not correspond to anything in the table of ARMA 
# We saw that there is a trend so we want to check the ACF after differencing of 1
# ACF shows peaks at multiple of 7 i.e. weekly seasonality

acf2(VT.train.bc)

acf2(diff(VT.train.bc,1))

acf2(diff(VT.train.bc,7))

acf2(diff(diff(VT.train.bc,7),1))

# Trying ARIMA
# We get a ARIMA(5,1,2) but the model is invalid (ACF of residuals and Ljung Box)

fit.arima <- auto.arima(VT.train.bc)
print(fit.arima)
m.arima <- sarima(VT.train.bc,3,1,3)

# Trying seasonal ARIMA
# From the ACF, we will need seasonal and differencing components
# s.arima1 seems slightly better

s.arima1 <- sarima(VT.train.bc,2,1,1,0,1,1,7)
s.arima2 <- sarima(VT.train.bc,2,1,1,1,1,1,7)
s.arima3 <- sarima(VT.train.bc,2,1,2,0,1,1,7)
s.arima4 <- sarima(VT.train.bc,2,1,2,1,1,1,7)

options(digits = 10)
cat("ARIMA(2,1,1)(0,1,1)[7] - AIC:",s.arima1$AIC," BIC:",s.arima1$BIC,"\n")
cat("ARIMA(2,1,1)(1,1,1)[7] - AIC:",s.arima2$AIC," BIC:",s.arima2$BIC,"\n")
cat("ARIMA(2,1,2)(0,1,1)[7] - AIC:",s.arima3$AIC," BIC:",s.arima2$BIC,"\n")
cat("ARIMA(2,1,2)(1,1,1)[7] - AIC:",s.arima4$AIC," BIC:",s.arima2$BIC,"\n")

validationsarima <- VT.valid.bc
trainingsarima <- VT.train.bc
dm<-0
er<-0
for (i in 1:730){
  s.arima1.for <- sarima.for(head(trainingsarima,n=2189+i), n.ahead=1,
                             p=2,d=1,q=2,P=1,D=1,Q=1,S=7)
  dm<-rbind(dm,s.arima1.for$pred)
  er<-rbind(er,s.arima1.for$se)
  trainingsarima <- ts(rbind(trainingsarima,validationsarima[i]))
}
dm <- ts(window(dm,start=2))
er <- ts(window(er,start=2))
validationsarima <-ts(validationsarima@.Data)

# Plotting the transformed data (to avoid playing with s.e)

ylims <- c(min(dm-2.2*er,VT.valid.bc),
           max(dm+2.2*er,VT.valid.bc))
xlims <- c(0,800)
plot(dm,type="p",pch=19,col="blue",ylab="Transformed Vermont Daily Demand",
     axes=F,ylim=ylims,xlim=xlims,xlab="Days since 2017-01-01",main="Transformed forecasts using seasonal ARIMA")
axis(2);axis(1)
lines(dm-1.96*er,lty=3,col="red");lines(dm+1.96*er,lty=3,col="green")
points(validationsarima,pch=13)
legend("bottomright",legend=c("ARIMA(2,1,2)(1,1,1)[7]","observed"),
       pch=c(19,13),col=c("blue","black"))
legend("bottomleft",
       legend=paste("MAPE =",round(accuracy(dm,VT.valid.bc$TS.1)[5],3)),
       pch=19, col="blue")

pred.sarima <- ((dm*lambda)+1)^(1/lambda)
pred.sarima <- timeSeries(pred.sarima,as.character(df.valid_regress$Date))

se.sarima <- ((er*lambda)+1)^(1/lambda)
se.sarima <- timeSeries(se.sarima,as.character(df.valid_regress$Date))

accuracy(pred.sarima$TS.1,VT.valid)
pred.sarima$TS.1

# ARX(1) model with 1 lag of y and HDD and CDD as exogeneous variables 
...
FH <- ts(FH)
VH <- ts(VH)
DMon <- ts(ifelse(dow==2,1,0))
DTue <- ts(ifelse(dow==3,1,0))
DWed <- ts(ifelse(dow==4,1,0))
DThu <- ts(ifelse(dow==5,1,0))
DFri <- ts(ifelse(dow==6,1,0))
DSat <- ts(ifelse(dow==7,1,0))

Vermont <- ts(Vermont)
#Validation set starting from 2016.12.31 just to avoid the NaN for the first value of lag1 y
Vermont.valid <- Vermont[2190:2920]
CDD.valid <- CDD[2190:2920]
CDD.1.valid <- CDD.1[2190:2920]
CDD.2.valid <- CDD.2[2190:2920]
HDD.valid <- HDD[2190:2920]
HDD.2.valid <- HDD.2[2190:2920]
wc.valid <- wc[2190:2920]
HI.valid<- HI[2190:2920]
FH.valid <- FH[2190:2920]
VH.valid <- VH[2190:2920]
DMon.valid <- DMon[2190:2920]
DTue.valid <- DTue[2190:2920]
DWed.valid <- DWed[2190:2920]
DThu.valid <- DThu[2190:2920]
DFri.valid <- DFri[2190:2920]
DSat.valid <- DSat[2190:2920]

n <-length(VT.valid)
for.arx1<-numeric()

for(i in 1:n){
  Vermont.train <- Vermont[1:2189+i]
  HDD.train <- HDD[1:2189+i]
  HDD.2.train <- HDD.2[1:2189+i]
  CDD.train <- CDD[1:2189+i]
  CDD.1.train <- CDD.1[1:2189+i]
  CDD.2.train <- CDD.2[1:2189+i]
  wc.train <- wc[1:2189+i]
  HI.train <- HI[1:2189+i]
  FH.train <- FH[1:2189+i]
  VH.train <- VH[1:2189+i]
  DMon.train <- DMon[1:2189+i]
  DTue.train <- DTue[1:2189+i]
  DWed.train <- DWed[1:2189+i]
  DThu.train <- DThu[1:2189+i]
  DFri.train <- DFri[1:2189+i]
  DSat.train <- DSat[1:2189+i]
  df.arx1 <- list(Vermont.train=Vermont.train,
                  HDD.train=HDD.train,
                  CDD.train=CDD.train,
                  HDD.2.train=HDD.2.train,
                  CDD.1.train=CDD.1.train,
                  CDD.2.train=CDD.2.train,
                  wc.train=wc.train,
                  HI.train=HI.train,
                  FH.train=FH.train,
                  VH.train=VH.train,
                  DMon.train=DMon.train,
                  DTue.train=DTue.train,
                  DWed.train=DWed.train,
                  DThu.train=DThu.train,
                  DFri.train=DFri.train,
                  DSat.train=DSat.train )
  m.arx1 <- dynlm(Vermont.train~HDD.train+CDD.train+
                    HDD.2.train+CDD.1.train+CDD.2.train+
                    wc.train+HI.train+FH.train+VH.train+
                    DMon.train+DTue.train+DWed.train+DThu.train+DFri.train+DSat.train+
                    L(Vermont.train,1),data=df.arx1)
  int[i] <- coef(m.arx1)[1]
  betaHDD[i] <- coef(m.arx1)[2]
  betaCDD[i] <- coef(m.arx1)[3]
  betaHDD.2[i] <- coef(m.arx1)[4]
  betaCDD.1[i] <- coef(m.arx1)[5]
  betaCDD.2[i] <- coef(m.arx1)[6]
  betawc[i] <- coef(m.arx1)[7]
  betaHI[i] <- coef(m.arx1)[8]
  betaFH[i]<- coef(m.arx1)[9]
  betaVH[i]<- coef(m.arx1)[10]
  betaDMon[i] <- coef(m.arx1)[11]
  betaDTue[i] <- coef(m.arx1)[12]
  betaDWed[i] <- coef(m.arx1)[13]
  betaDThu[i] <- coef(m.arx1)[14]
  betaDFri[i] <- coef(m.arx1)[15]
  betaDSat[i] <- coef(m.arx1)[16]
  phi[i] <- coef(m.arx1)[17]
  d <- i+1
  for.arx1[i] <- int[i]+ 
    betaHDD[i]*HDD.valid[d] + betaCDD[i]*CDD.valid[d] +
    betaHDD.2[i]*HDD.2.valid[d] + betaCDD.1[i]*CDD.1.valid[d] +betaCDD.2[i]*CDD.2.valid[d] +
    betawc[i]* wc.valid[d] + betaHI[i]* HI.valid[d] +
    betaFH[i]*FH.valid[d]+betaVH[i]*VH.valid[d] +
    betaDMon[i] * DMon.valid[d]+betaDTue[i] * DTue.valid[d] +
    betaDWed[i] * DWed.valid[d] +betaDThu[i] * DThu.valid[d] +
    betaDFri[i] * DFri.valid[d] +betaDSat[i] * DSat.valid[d] +
    phi[i]*Vermont.valid[d-1]
}

accuracy(for.arx1,VT.valid)
print(m.arx1)
...
test1<-window(VT.day.ts1,start=c(418,2),end=c(469,2))
training2 <- window(VT.day.ts1,end=c(418,1))

mnm<-ets(training1,allow.multiplicative.trend=T)
print(mnm)

trainingets <- training2
for (i in 1:365) {
  fore <- forecast(head(VT.day.ts1,n=2920+i),model=mnm,h=1,use.initial.values=T)
  dm<-ts(fore$mean)
  trainingets <- ts(c(trainingets,dm),start = start(trainingets),frequency = frequency(trainingets))
}


predets <- window(trainingets,start=c(418,2),end=c(469,2))
